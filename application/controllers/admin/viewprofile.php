...
class Viewprofile extends CI_Controller {
	  ...
	  {
		 if(is_array($this->session->userdata('userdata'))){
			 $user_data = $this->session->userdata('userdata');
		 $data['user'] = $this->model_user->getuserbyid($user_data['id']);
	  ...
	  {
		  $check=$this->model_object->getAllFromWhereOrder('city',"country_id ='".$this->input->post('country_id')."'",'city_id');
		//  print_r($check);
		  $str='<option>Select</option>';
		  if($check){
		  foreach($check as $ch)
		  {
			   $str.='<option  value="'.$ch->city_id.'">'.$ch->city_name.'</option>';
		  }
	  }
	  echo $str;
	  }
